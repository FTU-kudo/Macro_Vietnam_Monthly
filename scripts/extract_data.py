...
Yêu cầu:
1. Extract tất cả chỉ số có trong các nguồn trên (tối đa 41 chỉ số).
2. Áp dụng 4 rules (Time/Frequency/Conflict/Unit) đã nêu trong system prompt.
3. Bỏ qua chỉ số thiếu data — KHÔNG tạo placeholder.
4. Với mỗi chỉ số Cấp A (CPI, PMI, IIP, XNK, FDI, LNH, tỷ giá, tín dụng, vàng), viết narrative 2-4 câu.
5. Trả về JSON hợp lệ theo schema đã định nghĩa.
